...
//! shutdown. Engine panic mid-cycle is a recorded finding; the rig continues with a
//! fresh engine. Metrics: JSON lines to --out (append+flush, crash-safe).
//!
...
//!   --prior /tmp/esu6-2026-07-30.fftlog --prior /tmp/esu6-2026-07-31.fftlog \
//!   --speed 64 --out perf-runner/results/<date>-m7-soak.jsonl --max-hours 24
//! ```
//!
//! Week-long (unbounded cycles, 168 h wall):
//! ```text
//! cargo run --release -p fft-ui --bin m7-soak -- \
//!   --replay /tmp/esu6-wed-v3-ckpt.fftlog \
//!   --prior /tmp/esu6-2026-07-27.fftlog --prior /tmp/esu6-2026-07-28.fftlog \
...
//!   --prior /tmp/esu6-2026-07-27.fftlog --prior /tmp/esu6-2026-07-28.fftlog \
//!   --prior /tmp/esu6-2026-07-30.fftlog --prior /tmp/esu6-2026-07-31.fftlog \
//!   --speed 64 --cycle-secs 30 --max-cycles 3 --out /tmp/m7-soak-smoke.jsonl
//! ```
//!
...
use util::{RSS_BUDGET_BYTES, event_time_bounds};
...
fn main() {
    ...
    loop {
        ...
        if result.peak_rss_kb * 1024 > RSS_BUDGET_BYTES {
            rss_ceiling_fails += 1;
            eprintln!(
        ...
        rss_series.push(result.line.vm_rss_kb);
        if cycle == 3 {
            baseline_rss = Some(result.line.vm_rss_kb);
            eprintln!(
                "m7-soak: leak baseline (cycle 3) VmRSS={} kB",
                result.line.vm_rss_kb
            );
        }
        if let Some(base) = baseline_rss
            && leak_suspect(&rss_series, base, args.leak_window, args.leak_pct)
        {
            leak_suspects += 1;
            let window = &rss_series[rss_series.len() - args.leak_window..];
            eprintln!(
                "m7-soak: LEAK SUSPECT cycle={cycle} baseline_kb={base} \
                 window_kb={window:?} leak_pct={}",
                args.leak_pct
            );
        }
        ...
        eprintln!(
            "m7-soak: cycle={cycle} ok={} wall={:.1}s applied={} pubs={} seeks_exec={} \
             prior_ok={} prior_skip={} sessions={} rss={} kB hwm={} kB panic={:?}",
            result.line.ok,
    ...
    let summary = SummaryLine {
        ...
        label: args.label,
        notes: Some(format!(
            "rss_series_kb={rss_series:?}; signal=none (JSONL append-safe)"
        )),
    };
    append_json_line(&args.out, &summary);
    eprintln!(
        "m7-soak: SUMMARY cycles={cycle} failures={failures} leak_suspects={leak_suspects} \
         rss_ceiling_fails={rss_ceiling_fails} wall={:.1}s peak_rss={} kB peak_hwm={} kB out={}",
        summary.wall_secs,
        peak_vm_rss_kb,
        peak_vm_hwm_kb,
        args.out.display()
    ...
    if failures > 0 || rss_ceiling_fails > 0 {
        exit(1);
    }
}
